...
import {
  ...
  AuthHeaders,
  AUTH_HEADERS_REQUEST_SUCCEEDED,
  AuthHeadersSucceededAction,
} from "./types";
...
export const authHeadersRequestSucceeded = (
  authHeaders: AuthHeaders
): AuthHeadersSucceededAction => ({
  type: AUTH_HEADERS_REQUEST_SUCCEEDED,
  payload: {
    authHeaders,
  },
});
...
const generateAuthActions = (config: { [key: string]: any }): ActionsExport => {
  ...
  const platformStorage =
    Platform.OS == "web" ? AsyncLocalStorage : RNAsyncStorage;

  const Storage: DeviceStorage = Boolean(storage && storage.flushGetRequests)
    ? storage
    : platformStorage;
  ...
  const verifyToken = (verificationParams: VerificationParams) =>
    async function (dispatch: Dispatch<{}>): Promise<void> {
      ...
      try {
        ...
        dispatch(authHeadersRequestSucceeded(response.headers));
